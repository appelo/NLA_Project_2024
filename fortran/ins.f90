...
module problem_setup
  ...
  real(dp), parameter :: nu = 1.0_dp/100.0_dp
  real(dp), parameter :: pi = acos(-1.d0)
  logical, parameter :: do_plot = .true.
  logical, parameter :: dense_LA = .true.
...
module matrices
  ...
  real(dp), allocatable, dimension(:) :: pvec,pbvec,lpvec,uvec,vvec,ubvec,vbvec
...
contains
  ...
  subroutine afun(Ax,x,n)
    use type_defs
    implicit none
    integer, intent(in) :: n
    real(dp), intent(inout) :: Ax(n)
    real(dp), intent(inout) ::  x(n)
    integer :: i

    Ax(1) =  2.0_dp*x(2) - 2.0_dp*x(1)
    Ax(n) =       - 2.0_dp*x(n) + 2.0_dp*x(n-1)
    do i = 2,n-1
       Ax(i) =  x(i+1) - 2.0_dp*x(i) + x(i-1)
    end do
...
contains
  ...
  subroutine cg4p(x,b,nx,ny,hx,hy,k)
    use type_defs
    use afuns
    implicit none
    integer, intent(in) :: nx,ny
    real(dp), intent(inout)  :: hx,hy,k
    real(dp), intent(inout)  :: x((nx+1)*(ny+1))
    real(dp), intent(inout)  :: b((nx+1)*(ny+1))
    real(dp), dimension((nx+1)*(ny+1)) :: r,p,Ax,w

  end subroutine cg4p

  subroutine cg4uv(x,b,nx,ny,hx,hy,l)
    use type_defs
    use problem_setup, only : nu, k
    use afuns
    implicit none
    integer, intent(in) :: nx,ny
    real(dp), intent(inout)  :: hx,hy,l
    real(dp), intent(inout)  :: x((nx-1)*(ny-1))
    real(dp), intent(inout)  :: b((nx-1)*(ny-1))
    real(dp), dimension((nx-1)*(ny-1)) :: r,p,Ax,w

  end subroutine cg4uv
...
program ins
  ...
  ! hx=Lx/Nx, hy = Ly/Ny.
  real(dp) :: hx,hy,time1,time2,vx,vy,uv(2)
  integer :: i,j,sys_size_p,sys_size_pbig,info,nt,sys_size_uv,ivorz
  integer :: no_iter
  character(100) :: str
  integer :: nnz_p,nnz_uv,nnz_mult,iopt,ldb,nrhs,nvars
  logical :: colfirst
  integer*8 :: factor_p,factor_uv
  ...
  allocate(pvec((Nx+1)*(Ny+1)),pbvec((Nx+1)*(Ny+1)),lpvec((Nx+1)*(Ny+1)))
  allocate(pbvecbig((Nx+1)*(Ny+1)+1),pvecbig((Nx+1)*(Ny+1)+1))
  allocate(uvec((Nx-1)*(Ny-1)),vvec((Nx-1)*(Ny-1)),ubvec((Nx-1)*(Ny-1)),vbvec((Nx-1)*(Ny-1)))
  ...
  do nt = 1,Nsteps
     ...
     call setupRhsideUV(Ubvec,Vbvec,nx,ny,hx,hy,gux,guy,gvx,gvy,&
     ...
     if (dense_LA) then
        ! !!! YOUR CODE REPLACES THIS !!!!
        CALL DGETRS('N',sys_size_uv,1,Lapuv,sys_size_uv,IPIV_uv,ubvec,sys_size_uv,INFO)
        CALL DGETRS('N',sys_size_uv,1,Lapuv,sys_size_uv,IPIV_uv,vbvec,sys_size_uv,INFO)
        ! !!! END YOUR CODE REPLACES THIS !!!!
        uvec = ubvec
        vvec = vbvec
     else
        ! Put your code here
        ! call cg4uv(uvec,ubvec,nx,ny,hx,hy,no_iter)
        ! write(*,*) 'U-solve number of iterations: ',no_iter
        ! call cg4uv(vvec,vbvec,nx,ny,hx,hy,no_iter)
        ! write(*,*) 'V-solve number of iterations: ',no_iter
     end if
     ! Swap long vector into 2D array
     do j = 1,ny-1
        do i = 1,nx-1
     ...
     ! Plot stuff
     if (do_plot) then
        if ((mod(nt,nplot) .eq. 0 ).or. (nt .eq. nsteps)) then
           write(*,*) "plotting at timstep ", nt
     ...
     if (maxval(abs(u(0:nx,0:ny))) .gt. 1.0d6) then
        ...
        write(*,*) k,k*maxval(sqrt(u**2+v**2)),hx
        stop 123
     end if
     if (mod(nt,100).eq.0) then
        write(*,*) "Timestep ", nt, " out of ", nsteps
        write(*,*) k*maxval(sqrt(u**2+v**2))/hx
     end if
...
!     stop 112
  ...
  call printdble2d(u,-1,nx+1,-1,ny+1,'restartu.txt')
  call printdble2d(v,-1,nx+1,-1,ny+1,'restartv.txt')
...
subroutine updateBCforU(u,v,gux,guy,gvx,gvy,nx,ny)
  ...
  ! Fix up corners by taking averages
  u(0,0)   = 0.5_dp*(gux(1,0) + guy(1,0))
  u(0,ny)  = 0.5_dp*(gux(1,ny) + guy(2,0))
  u(nx,0)  = 0.5_dp*(gux(2,0) + guy(1,nx))
  u(nx,ny)  = 0.5_dp*(gux(2,ny) + guy(2,nx))
  v(0,0)   = 0.5_dp*(gvx(1,0) + gvy(1,0))
  v(0,ny)  = 0.5_dp*(gvx(1,ny) + gvy(2,0))
  v(nx,0)  = 0.5_dp*(gvx(2,0) + gvy(1,nx))
  v(nx,ny)  = 0.5_dp*(gvx(2,ny) + gvy(2,nx))
...
subroutine computeAndUpdateGPforU(u,v,hx,hy,nx,ny)
  ...
  v(-1,0:ny)   = vbx(1,0:ny)
  v(nx+1,0:ny) = vbx(2,0:ny)
  u(0:nx,-1)   = uby(1,0:nx)
  u(0:nx,ny+1) = uby(2,0:nx)
...
subroutine computeGPforP(pbx,pby,u,v,gux,guy,gvx,gvy,hx,hy,nu,nx,ny)
  ...
  pby(1,0:nx) = -2.0_dp*hy*(&
       ...
       +nu*(&
       - (u(1:nx+1,1)-u(-1:nx-1,1)-u(1:nx+1,-1)+u(-1:nx-1,-1))/(4.0_dp*hy*hx) &
  ...
  pby(2,0:nx) = +2.0_dp*hy*(&
       ...
       +nu*(&
       - (u(1:nx+1,ny+1)-u(1:nx+1,ny-1)-u(-1:nx-1,ny+1)+u(-1:nx-1,ny-1))/(4.0_dp*hy*hx) &
